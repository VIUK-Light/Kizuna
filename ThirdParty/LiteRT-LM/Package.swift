...
let package = Package(
  ...
  targets: [
    // Official LiteRT-LM v0.14.0 iOS runtime.  Keep the wrapper in this
    // repository, but pin the native artifact to the matching upstream release
    // so Swift and C APIs are always updated together.
    .binaryTarget(
      name: "CLiteRTLM",
    ...
    // The Swift wrapper is synchronised from the same v0.14.0 source release.
    .target(
      name: "LiteRTLM",
      dependencies: [
