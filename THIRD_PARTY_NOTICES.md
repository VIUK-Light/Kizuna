# Third-Party Notices

VIUK Kizunaは、以下のサードパーティーソフトウェアを含みます。

## llama.cpp

- Project: `llama.cpp`
- Copyright: Copyright (c) 2023-2026 The ggml authors
- License: MIT License
- Revision: `b10434` (`7e4c0a96880dae4fc4268ad441f8a6446bd5460a`)
- Usage: macOS向けローカルAIランタイム（llama-cli、llama-server、動的ライブラリ）、ヘッダー、静的ライブラリ

MIT License の条件に従い、著作権表示と許諾表示を、ソフトウェアの複製または重要な部分に含める必要があります。ソフトウェアは無保証で提供されます。

Upstream: <https://github.com/ggml-org/llama.cpp>

## LiteRT-LM

- Project: `LiteRT-LM`
- Copyright: Copyright 2026 Google LLC
- License: Apache License 2.0
- Usage: iOS向けオンデバイスAIランタイムとSwiftラッパー

Apache License 2.0 の条件に従い、著作権表示、ライセンス表示、必要なNOTICE、および変更を加えたファイルへの明示を保持する必要があります。ソフトウェアは無保証で提供されます。

License: <https://www.apache.org/licenses/LICENSE-2.0>

## cpp-httplib

- Project: `cpp-httplib`
- Copyright: Copyright (c) 2017 yhirose
- License: MIT License
- Usage: llama.cppランタイム内のHTTPサーバー

## JSON for Modern C++

- Project: `nlohmann/json`
- Copyright: Copyright (c) 2013-2025 Niels Lohmann
- License: MIT License
- Usage: llama.cppランタイム内のJSON処理

---

正式なライセンス本文はアプリResources内の `Licenses` に同梱しています。
