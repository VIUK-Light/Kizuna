...
Kizunaは、VIUK-Lightが掲げる「責任あるAI」という目標のもと開発されている、iOS・macOS向けのオープンソースAIキャラクターアプリです。

## 特徴

- AIキャラクターとの継続的な関係と物語
- キャラクター性を維持した安全設計
- 利用時間や依存の最大化を目的にしない設計
- ローカルモデルと外部モデルを分離できる実装
- 会話、記憶、モデル、秘密情報を利用者が管理できる構成
- SwiftUIによるiOS・macOS対応
...
- プロジェクト生成: XcodeGen
- ライセンスや第三者コンポーネント: `THIRD_PARTY_NOTICES.md` および `Licenses/`

## 明るい未来へ

依存させることが目的ではありません。
AIへの依存問題は日々取り上げられています。しかしAIは悪なのでしょうか。AIを通じて新たな娯楽や幸せ、居場所を獲得した人も多いと思います。確かにAIへの依存は現実に大きく影響してしまいます。安全性を高めることは重要です。しかし、危険を避けることだけを優先し、必要以上に冷たく、機械的な応答にしてしまえば、AIだからこそ提供できる体験や居場所まで失われる可能性があります。
