...
struct StoryPromptBuilder {
    ...
    func build(
        ...
        push(copy("本文の後に、場所・時間・天候・関係・キャラ状態・所持品が今回明確に変化した時だけ、表示から除去される1行の `STATE_UPDATE: {JSON}` を付ける。キーは location,timeOfDay,mood,weather,relationshipStage,characterUpdates,inventoryChanges,activeGoals。変化がなければ付けない。付ける場合は、表示本文または今回のユーザー発言から1〜120文字の完全一致する短い引用を `evidence` に入れる。", "After the dialogue, only when a state clearly changes, append one hidden `STATE_UPDATE: {JSON}` line. Use keys location,timeOfDay,mood,weather,relationshipStage,characterUpdates,inventoryChanges,activeGoals; omit it when nothing changed. When present, include an exact 1-120 character quote from the visible reply or the current user message in `evidence`."))
    ...
    func buildLocalRuntimePrompt(
        ...
        var lines = [
            ...
            isEnglish
                ? "Start with \(npcSpeakerLabel): a natural reply. \(duplicateInstruction) Add one Narration: text line only when the location, time, or goal changes or the user explicitly asks for it. Do not repeat the previous scene, greeting, or reply. Never invent the user's dialogue, actions, or feelings. If location, weather, relationship, character state, or inventory clearly changes, append one hidden STATE_UPDATE: {JSON} line after the dialogue using only the documented keys; omit it when nothing changed. When present, include an exact 1-120 character quote from the visible reply or current user message in evidence."
                : "基本は「\(npcSpeakerLabel): 自然な返事」を1行だけ返す。\(duplicateInstruction) 場所・時間・目的が実際に変化した時、またはユーザーが明示した時だけ、その前に短い「ナレーション: 本文」を1行添える。毎ターンの場面説明、直前と同じ情景・挨拶・返答は禁止。ユーザーの台詞・行動・感情は代弁しない。場所・天候・関係・キャラ状態・所持品が明確に変化した時だけ、本文の後に `STATE_UPDATE: {JSON}` を1行付ける。変化がなければ付けない。付ける場合は、表示本文または今回のユーザー発言から1〜120文字の完全一致する短い引用を `evidence` に入れる。",
            isEnglish
