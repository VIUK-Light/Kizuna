...
struct StoryPromptBuilder {
    ...
    func build(
        ...
        // 出来事はカードや追加生成ではなく、通常本文の隠しSTATE_UPDATEに
        // 任意で含める。発生間隔と状態遷移はアプリ側Reducerで検証する。
        sections.append(
            "## \(copy("自然発生イベント", "Spontaneous events"))\n"
                + StoryEventPolicy.production.promptInstruction(
                    for: session,
                    userInput: userInput,
                    isEnglish: isEnglish
                )
        ...
        push(copy("本文の後に、場所・時間・天候・関係・キャラ状態・所持品が今回明確に変化した時だけ、表示から除去される1行の `STATE_UPDATE: {JSON}` を付ける。キーは location,timeOfDay,mood,weather,relationshipStage,characterUpdates,inventoryChanges,activeGoals,eventUpdate。変化がなければ付けない。", "After the dialogue, only when a state clearly changes, append one hidden `STATE_UPDATE: {JSON}` line. Use keys location,timeOfDay,mood,weather,relationshipStage,characterUpdates,inventoryChanges,activeGoals,eventUpdate; omit it when nothing changed."))
    ...
    func buildLocalRuntimePrompt(
        ...
        var lines = [
            ...
            isEnglish
                ? "Start with \(npcSpeakerLabel): a natural reply. \(duplicateInstruction) Add one Narration: text line only when the location, time, or goal changes or the user explicitly asks for it. Do not repeat the previous scene, greeting, or reply. Never invent the user's dialogue, actions, or feelings. If location, weather, relationship, character state, inventory, or a permitted spontaneous event clearly changes, append one hidden STATE_UPDATE: {JSON} line after the dialogue using only the documented keys; omit it when nothing changed."
                : "基本は「\(npcSpeakerLabel): 自然な返事」を1行だけ返す。\(duplicateInstruction) 場所・時間・目的が実際に変化した時、またはユーザーが明示した時だけ、その前に短い「ナレーション: 本文」を1行添える。毎ターンの場面説明、直前と同じ情景・挨拶・返答は禁止。ユーザーの台詞・行動・感情は代弁しない。場所・天候・関係・キャラ状態・所持品、または許可された自然発生イベントが明確に変化した時だけ、本文の後に `STATE_UPDATE: {JSON}` を1行付ける。変化がなければ付けない。"
        ]
        // 1,250 UTF-8 byte制限に入る前の先頭へ置き、ioriでもイベントの
        // 発生条件と「話題を変えたら追わない」条件が削られないようにする。
        lines.append(
            StoryEventPolicy.production.promptInstruction(
                for: session,
                userInput: userInput,
