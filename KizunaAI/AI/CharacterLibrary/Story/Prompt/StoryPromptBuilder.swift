...
struct StoryPromptBuilder {
    ...
    func build(
        ...
        sections.append(
            ...
            場面描写は毎回書かないでください。場所・時間・人物の行動が実際に変わった時だけ、「ナレーション: 本文」を1行だけ添えます。
            """
        )
        ...
        if let userCharacterName {
            sections.append(
                ...
                ユーザーの返答が必要な場面では、相手役の短い言葉や沈黙で止めます。
                """
            )
        }
        ...
        push("出力は通常1行だけの「NPC名: 発話」。場面が実際に変化した時だけ、先に短い「ナレーション: 本文」を1行添える。既に示した場所・天気・光景を言い換えて繰り返さない。")
        ...
        push("ユーザーの短い返事にも、直前の発話を受けた具体的な反応を返す。同じ挨拶、同じ質問、同じ情景描写を繰り返さない。")
        ...
        sections.append(
            """
            ## 出力開始
            基本は「\(speakerHint): 発話」を1行だけ出す。
            場面の変化を示す必要がある時だけ、その前に「ナレーション: 本文」を1行添える。ユーザー操作キャラの名前は使わない。
            """
        )
    ...
    func buildLocalRuntimePrompt(
        ...
        var lines = [
            ...
            "基本は1行だけ「\(npcName): 自然な返事」を返す。ユーザーの台詞・行動・感情は代弁しない。",
            "ナレーションは、場所・時間・行動が実際に変わった時だけ短く1行添える。毎ターンの情景説明、直前と似た景色の言い換え、同じ挨拶や質問の繰り返しは禁止。"
        ]
